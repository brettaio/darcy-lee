import { Component } from '@angular/core';
import { appDataStore } from '../../../../websites/enzo-concrete/src/app/store/app-data.store';
import { CtaButtonComponent } from '../../components';
import { CtaButtonAltComponent } from '../cta-button-alt/cta-button-alt.component';

@Component({
  selector: 'component-hero-enzo',
  imports: [CtaButtonComponent, CtaButtonAltComponent],
  template: `
    <div class="bg-white h-screen">
      <!-- 1) Full viewport height -->
      <main class="relative isolate h-full">
        <!-- 2) Let child fill parent -->
        <!-- Your background SVG -->
        <svg
          class="absolute inset-x-0 top-0 -z-10 h-[64rem] w-full stroke-gray-200 [mask-image:radial-gradient(32rem_32rem_at_center,white,transparent)]"
          aria-hidden="true"
        >
          <!-- ... unchanged defs/pattern code ... -->
        </svg>

        <!-- The blur shape -->
        <div
          class="absolute top-0 right-0 left-1/2 -z-10 -ml-24 transform-gpu overflow-hidden blur-3xl lg:ml-24 xl:ml-48"
          aria-hidden="true"
        >
          <div
            class="aspect-801/1036 w-[50.0625rem] bg-linear-to-tr from-[#ff80b5] to-[#9089fc] opacity-30"
            style="clip-path: polygon(63.1% 29.5%, 100% 17.1%, 76.6% 3%, 48.4% 0%, 44.6% 4.7%, 54.5% 25.3%, 59.8% 49%, 55.2% 57.8%, 44.4% 57.2%, 27.8% 47.9%, 35.1% 81.5%, 0% 97.7%, 39.2% 100%, 35.2% 81.4%, 97.2% 52.8%, 63.1% 29.5%)"
          ></div>
        </div>

        <!-- Main content container -->
        <div class="overflow-hidden h-full flex items-center">
          <!-- 3) 'flex items-center' for vertical centering -->
          <div class="mx-auto max-w-7xl px-6 lg:px-8 w-full">
            <!-- 4) Remove large 'pt-60' or 'pb-16' so we rely on full height -->
            <div
              class="mx-auto max-w-2xl gap-x-14 lg:mx-0 lg:flex lg:max-w-none lg:items-center"
            >
              <!-- TEXT AREA -->
              <div class="relative w-full lg:max-w-xl lg:shrink-0 xl:max-w-2xl">
                <div
                  class="text-5xl font-semibold tracking-tight text-pretty text-gray-900 sm:text-7xl"
                  [innerHTML]="appDataStore.homeData().heroH2"
                ></div>
                <div
                  class="mt-8 text-lg font-medium text-pretty text-gray-500 sm:max-w-md sm:text-xl/8 lg:max-w-none"
                  [innerHTML]="appDataStore.homeData().heroParagraph"
                ></div>
                <div
                  class="mt-10 flex items-center md:justify-left justify-center gap-x-6 xs:hidden"
                >
                  <component-cta-button
                    customClasses="text-lg text-slate-100 font-semibold"
                  ></component-cta-button>
                  <component-cta-button-alt
                    customClasses="text-lg font-bold text-slate-600"
                  ></component-cta-button-alt>
                </div>
              </div>

              <!-- IMAGES AREA -->
              <div
                class="mt-14 flex justify-end gap-8 sm:-mt-44 sm:justify-start sm:pl-20 lg:mt-0 lg:pl-0"
              >
                <div
                  class="ml-auto w-44 flex-none pt-32 sm:ml-0 sm:pt-80 lg:order-last lg:pt-36 xl:order-none xl:pt-80"
                >
                  <div class="relative">
                    <img
                      [src]="appDataStore.heroData().heroImage1"
                      alt=""
                      class="aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg"
                    />
                    <div
                      class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-gray-900/10 ring-inset"
                    ></div>
                  </div>
                </div>
                <div
                  class="mr-auto w-44 flex-none space-y-8 sm:mr-0 sm:pt-52 lg:pt-36"
                >
                  <div class="relative mb-4">
                    <img
                      [src]="appDataStore.heroData().heroImage2"
                      alt=""
                      class="aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg"
                    />
                    <div
                      class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-gray-900/10 ring-inset"
                    ></div>
                  </div>
                  <div class="relative">
                    <img
                      [src]="appDataStore.heroData().heroImage3"
                      alt=""
                      class="aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg"
                    />
                    <div
                      class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-gray-900/10 ring-inset"
                    ></div>
                  </div>
                </div>
                <div class="w-44 flex-none pt-32 sm:pt-0">
                  <div class="relative mb-4">
                    <img
                      [src]="appDataStore.heroData().heroImage4"
                      alt=""
                      class="aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg"
                    />
                    <div
                      class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-gray-900/10 ring-inset"
                    ></div>
                  </div>
                  <div class="relative">
                    <img
                      [src]="appDataStore.heroData().heroImage5"
                      alt=""
                      class="aspect-2/3 w-full rounded-xl bg-gray-900/5 object-cover shadow-lg"
                    />
                    <div
                      class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-gray-900/10 ring-inset"
                    ></div>
                  </div>
                </div>
              </div>
              <!-- END IMAGES -->
            </div>
          </div>
        </div>
      </main>
    </div>
  `,
  styles: ``,
})
export class HeroEnzoComponent {
  appDataStore = appDataStore;
}
